...
GAIN_MODE_DT gstGainModeDt = {
    .stOvTab = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF}};
...
const ISP_CMOS_GAMMA_S gstIspGamma = // gamma 96
    {1, {0x000, 0x00d, 0x01a, 0x027, 0x034, 0x041, 0x04d, 0x059, 0x065, 0x070,
         0x07c, 0x087, 0x092, 0x09d, 0x0a7, 0x0b2, 0x0bc, 0x0c7, 0x0d1, 0x0da,
         0x0e4, 0x0ee, 0x0f7, 0x100, 0x109, 0x112, 0x11b, 0x124, 0x12d, 0x135,
         0x13d, 0x146, 0x14e, 0x15d, 0x16d, 0x17c, 0x18a, 0x198, 0x1a6, 0x1b4,
         0x1c1, 0x1ce, 0x1da, 0x1e6, 0x1f2, 0x1fe, 0x209, 0x214, 0x21f, 0x233,
         0x247, 0x25a, 0x26c, 0x27e, 0x28f, 0x29f, 0x2ae, 0x2bd, 0x2cb, 0x2d9,
         0x2e6, 0x2f3, 0x2ff, 0x30b, 0x316, 0x321, 0x32c, 0x337, 0x341, 0x34a,
         0x354, 0x35d, 0x366, 0x36e, 0x377, 0x37f, 0x387, 0x38e, 0x396, 0x39d,
         0x3a4, 0x3ab, 0x3b2, 0x3b8, 0x3bf, 0x3c5, 0x3cb, 0x3d1, 0x3d7, 0x3dc,
         0x3e2, 0x3e7, 0x3ec, 0x3f2, 0x3f7, 0x3fc, 0x3ff}};
...
static XM_S32 cmos_get_isp_default(ISP_CMOS_DEFAULT_S *pstDef) {
  ...
  XM_MPI_ISP_Memncpy((XM_U8 *)&(pstDef->stAgcTbl), (XM_U8 *)pstAgcTab,
...
static XM_S32 cmos_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft) {
  ...
  if (ExtCfg_CheckFun(1, ROM_FUN_AEDEF) == XM_SUCCESS) {
    ...
    if (s32Ret != XM_FAILURE)
      pstAeSnsDft->u8AeCompensation = (XM_U8)s32Ret;
  }
...
XM_S32 cmos_set_sns_attr(ISP_CMOS_SNS_ATTR_S *pstSnsAttr) {
  ...
  XM_MPI_ISP_Memncpy((XM_U8 *)&gstSnsAttr, (XM_U8 *)pstSnsAttr,
                     sizeof(ISP_CMOS_SNS_ATTR_S));
  return XM_SUCCESS;
}
...
XM_S32 cmos_get_sns_attr(ISP_CMOS_SNS_ATTR_S *pstSnsAttr) {
  ...
  XM_MPI_ISP_Memncpy((XM_U8 *)pstSnsAttr, (XM_U8 *)&gstSnsAttr,
                     sizeof(ISP_CMOS_SNS_ATTR_S));
  return XM_SUCCESS;
}

XM_VOID cmos_gain_calc_table_com(XM_U32 u32InTimes,
...
static XM_S32 cmos_get_awb_default(AWB_SENSOR_DEFAULT_S *pstAwbSnsDft) {
  ...
  if (ExtCfg_CCM_Set(&(pstAwbSnsDft->stCcm)) != XM_SUCCESS) {
    XM_MPI_ISP_Memncpy((XM_U8 *)&(pstAwbSnsDft->stCcm), (XM_U8 *)pstCcm,
...
int sensor_register_callback(ISP_DEV IspDev) {
  ...
  IspDev = (IspDev < 0)
               ? 0
               : ((IspDev > (ISP_NUM_MAX - 1)) ? (ISP_NUM_MAX - 1) : IspDev);
  gu32AGainNow = 0;
  gu32AGainMin = 0;
  gu32DGainNow = 0;
  gu32DGainMin = 0;
  ...
  s32Ret = XM_MPI_ISP_SensorRegCallBack(
      (XM_S32)gSensorChip, (const ISP_SENSOR_REGISTER_S *const)&stIspRegister);
  if (s32Ret) {
    return s32Ret;
  }
  stLib.s32Id = (XM_S32)gSensorChip;
  XM_MPI_ISP_Memncpy((XM_U8 *)stLib.acLibName, (XM_U8 *)XM_AE_LIB_NAME,
                     sizeof(XM_AE_LIB_NAME));
  s32Ret = XM_MPI_AE_SensorRegCallBack(IspDev, &stLib, (XM_S32)gSensorChip,
                                       &stAeRegister);
  if (s32Ret) {
    return s32Ret;
  }
  ...
  stLib.s32Id = (XM_S32)gSensorChip;
  XM_MPI_ISP_Memncpy((XM_U8 *)stLib.acLibName, (XM_U8 *)XM_AWB_LIB_NAME,
                     sizeof(XM_AWB_LIB_NAME));
  s32Ret =
      XM_MPI_AWB_SensorRegCallBack(&stLib, (XM_S32)gSensorChip, &stAwbRegister);
  if (s32Ret) {
    return s32Ret;
  }
  return 0;
...
START_TAB:
  ...
  case SENSOR_CHIP_SP140A:
  case SENSOR_CHIP_SP2305:
    if ((u32SnsChipFilt != SENSOR_CHIP_SP140A) &&
        (u32SnsChipFilt != SENSOR_CHIP_SP2305)) {
      gSnsDevAddr = 0x3C;
      gSnsRegAddrByte = 0x1;
      gSnsRegDataByte = 0x1;
...
int sensor_get_chip(I2C_DATA_S *pstI2CData) {
  ...
  PrintHex(8, gSnsDevAddr);
  DBG(" ");
  PrintHex(8, gSnsRegAddrByte);
  DBG(" ");
  PrintHex(8, gSnsRegDataByte);
  DBG(" ");
...
void sensor_set_chip(XM_S32 s32SnsChip) { gSensorChip = s32SnsChip; }
...
void sensor_set_fps(XM_U8 channel, XM_U8 frame) { return; }

// u8Mode:  0:Normal  1:XM AHD  2: CVBS
void sensor_set_isp_para(XM_U8 u8Mode) { return; }

XM_S32 sensor_set_reslotionType(PROFILE enResType) { return XM_SUCCESS; }
