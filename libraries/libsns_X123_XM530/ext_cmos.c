...
void sensor_input_2(XM_U32 u32Addr, XM_U16 u16Num) {
  ...
  for (u16i = 0; u16i < u16Num; u16i++) {
    if ((u16Num > 2) && (u16i < u16Num - 2) &&
        (*(pu16Data + u16i * 2 + 0) == 0xF5) &&
        (*(pu16Data + u16i * 2 + 1) == 0xA5) &&
        (*(pu16Data + (u16i + 1) * 2 + 0) == 0x5A) &&
        (*(pu16Data + (u16i + 1) * 2 + 1) == 0x5F)) {
      u16i += 2;
      ...
      sensor_write_register((XM_U32) * (pu16Data + u16i * 2 + 0),
                            (XM_U32) * (pu16Data + u16i * 2 + 1));
...
void sensor_input_1(XM_U32 u32Addr, XM_U16 u16Num) {
  ...
  for (u16i = 0; u16i < u16Num; u16i++) {
    if ((u16Num > 2) && (u16i < u16Num - 2) &&
        (*(pu8Data + u16i * 2 + 0) == 0xF5) &&
        (*(pu8Data + u16i * 2 + 1) == 0xA5) &&
        (*(pu8Data + (u16i + 1) * 2 + 0) == 0x5A) &&
        (*(pu8Data + (u16i + 1) * 2 + 1) == 0x5F)) {
      u16i += 2;
      ...
      sensor_write_register((XM_U32) * (pu8Data + u16i * 2 + 0),
                            (XM_U32) * (pu8Data + u16i * 2 + 1));
...
int sensor_write_ex(XM_U8 u8Mode, XM_U32 u32Addr, XM_U16 u16Num) {
  ...
  if (u16Num > 0) {
    ...
    for (u16i = 0; u16i < u16Num; u16i++) {
      ...
      (*pFun_cfgRead)(gSnsRegAddrByte, u32Addr,
                      (XM_U8 *)&au16RegAddr[0]); // Addr
      (*pFun_cfgRead)(gSnsRegDataByte, u32Addr + gSnsRegAddrByte,
                      (XM_U8 *)&au16RegData[0]); // Data
      (*pFun_cfgRead)(gSnsRegAddrByte, u32Addr + u16Len,
                      (XM_U8 *)&au16RegAddr[1]); // Addr
      (*pFun_cfgRead)(gSnsRegDataByte, u32Addr + gSnsRegAddrByte + u16Len,
                      (XM_U8 *)&au16RegData[1]); // Data
      if ((u16Num > 2) && (u16i < u16Num - 2) && (au16RegAddr[0] == 0xF5) &&
          (au16RegData[0] == 0xA5) && (au16RegAddr[1] == 0x5A) &&
          (au16RegData[1] == 0x5F)) {
        u16i += 2;
        au16RegAddr[0] = au16RegData[0] = 0;
...
XM_S32 cmos_set_mirror_flip_ex(XM_U8 u8Mirror, XM_U8 u8Flip) {
  ...
  PrintInt(8, (XM_S32)gAppEncoderinfo);
  ENTER();
  PrintInt(8, (XM_S32)u8Mirror);
  PrintInt(8, (XM_S32)u8Flip);
  ENTER();
  PrintInt(8, (XM_S32)u16HOfst);
  PrintInt(8, (XM_S32)u16VOfst);
  ENTER();
#endif
...
XM_S32 cmos_get_ae_default_ext(AE_SENSOR_DEFAULT_V2_S *pstAeSnsDft) {
  ...
  PrintInt(8, pstAeSnsDft->u32MinIntTime);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32MaxIntTime);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32MinAgain);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32MaxAgain);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32MinDgain);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32MaxDgain);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32MaxISPDgain);
  ENTER();
  PrintHex(8, pstAeSnsDft->u32UpdateCfg);
  ENTER();
  PrintInt(8, gu8MaxShutterOfst);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32InitExp[0]);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32InitExp[1]);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32InitExp[2]);
  ENTER();
  PrintInt(8, pstAeSnsDft->u32InitExp[3]);
  ENTER();
  PrintHex(8, (XM_U32)pstAeSnsDft->u16AlgMode);
  ENTER();
#endif
  return 0;
}
...
static void cmos_gain_calc_table(XM_U8 u8GainMode, XM_U32 u32InTimes,
  ...
  if (u8Mode == 2) // Table
  {
    ...
    if (!(su8Flg & (1 << u8GainMode))) {
      ...
      PrintInt(8, (int)u8GainMode);
      ENTER();
      PrintInt(8, (int)u8Num);
      ENTER();
...
XM_VOID cmos_again_calc_table_ex(XM_U32 u32InTimes,
                                 AE_SENSOR_GAININFO_S *pstAeSnsGainInfo) {
  cmos_gain_calc_table(0, u32InTimes, pstAeSnsGainInfo);
  return;
}

XM_VOID cmos_dgain_calc_table_ex(XM_U32 u32InTimes,
                                 AE_SENSOR_GAININFO_S *pstAeSnsGainInfo) {
...
XM_VOID cmos_fps_set_ex(XM_U8 u8Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft) {
  ...
  if (ExtCfg_BaseAddrGet(0, ROM_FUN_FPS, &u32Addr) == XM_SUCCESS) {
    ...
    ExtCfg_Read_V2(1, u32Addr + 2, &u8IspRegNum);
    u32Len =
        (gSnsRegDataByte + gSnsRegAddrByte) * u8SnsRegNum + 8 * u8IspRegNum + 3;
    u32Addr += 3;
...
#ifdef DEBUG_ON
  PrintInt(8, (XM_U32)gu8Fps);
  ENTER();
  PrintInt(8, (XM_U32)gu16FullLines);
  ENTER();
...
void gainLogic_ex(XM_U32 u32AllGain) {
  ...
  if (((su8Idx & 0x0F) != u8FpsModeNow) || (((su8Idx >> 4) & 0x0F) != u8i)) {
    ...
    for (u8i = 0; u8i < u8RegNum; u8i++) {
      u32Reg = 0;
      ExtCfg_Read_V2(gSnsRegAddrByte, u32Addr + gSnsRegAddrByte * u8i,
                     (XM_U8 *)&u32Reg);
      u32Val = 0;
      ExtCfg_Read_V2(gSnsRegDataByte, u32AddrTmp + gSnsRegDataByte * u8i,
                     (XM_U8 *)&u32Val);
...
void shutLogic_ex(XM_U32 u32IntTime) {
  ...
  if (u32IntTime < u16ThrL) {
    ...
    u32AddrTmp = u32Addr + (XM_U32)u8RegNum * gSnsRegAddrByte +
                 (XM_U32)u8RegNum * gSnsRegDataByte;
  }
  if (u32AddrTmp != 0) {
    for (u8i = 0; u8i < u8RegNum; u8i++) {
      u32Reg = 0;
  ...
  // logic2
  if (u8RegNum2 != 0) {
    u32Addr += (XM_U32)u8RegNum * gSnsRegAddrByte +
               (XM_U32)u8RegNum * gSnsRegDataByte * 2;
