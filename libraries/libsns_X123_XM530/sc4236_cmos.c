  ...
  if (((su8Idx >> 4) & 0x0F) != u8Idx2) {
    ...
    for (u8i = 0; u8i < 4; u8i++) {
      sensor_write_register((XM_U32)gau16LogicAddr[u8i],
                            (XM_U32)gau8Logic_sc4236[u8Idx2][u8i]);
    }
    if (su8Idx != 0xFF) {
...
STATIC XM_VOID cmos_gains_update(XM_U32 u32Again, XM_U32 u32Dgain) {
  ...
  if ((gu32AGainNow != 0) && (gu32AGainMin != 0)) {
    ...
    u32AllGain =
        (XM_U64)gau16GainTbl_SmatSns[u32Again] * u32AllGain * 16 / 1024;
  }
...
XM_S32
cmos_init_sensor_exp_function_sc4236(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc) {
#if 0
...
const AE_SENSOR_DEFAULT_V2_S gstAeSnsDef_sc4236 = {2,
                                                   1500 -
                                                       4, // minInt   	maxInt
                                                   1024,
                                                   15872, // minAgain	maxAgain
                                                   128,
                                                   512, // minDgain     maxDgain
                                                   0xFFF,      // maxIspGain
                                                   0x01010002, // Update
                                                   {700, 700, 700, 700},
                                                   0x00};
...
const ISP_COLORMATRIX_AUTO_S g_stAwbCcm_sc4236 = {
    5000,
    {
        0x000, 356, -135, 35, 0x000, -49, 327, -22, 0x000, -5, -145,
        406 // 0x000, -35, -130, 421
    },
    4000,
