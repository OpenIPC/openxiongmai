...
static const ISP_CMOS_AGC_TABLE_S g_stIspAgcTable = {
    ...
    {0xA0, 0xA0, 0x9A, 0x90, 0x88, 0x78, 0x50, 0x50, 0x50, 0x10, 0x10, 0x10,
     0x10, 0x10, 0x10, 0x10},
    /* Blc	*/
    {0x46, 0x46, 0x46, 0x46, 0x46, 0x44, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40,
     0x40, 0x40, 0x40, 0x40},
    /*Y_Tran gamma*/
...
const static XM_U8 gau8Logic_sc3035[3][4] = {
    ...
    {0x67, 0x64, 0xd2, 0x02},
    {0x67, 0x62, 0x63, 0x06}};
const static XM_U16 gau16LogicAddr[4] = {0x3630, 0x3635, 0x3620, 0x3627};
  ...
  if (((su8Idx >> 4) & 0x0F) != u8Idx2) {
    ...
    for (u8i = 0; u8i < 4; u8i++) {
      sensor_write_register((XM_U32)gau16LogicAddr[u8i],
                            (XM_U32)gau8Logic_sc3035[u8Idx2][u8i]);
    }
  }
}
...
XM_S32
cmos_init_sensor_exp_function_sc3035(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc) {
