...
static XM_VOID cmos_again_calc_table(XM_U32 u32InTimes,
  ...
  pstAeSnsGainInfo->u32SnsTimes =
      gau16GainTbl_SmatSns[pstAeSnsGainInfo->u32GainDb];
  return;
}
...
static void gainLogic_sc2145H(XM_U32 u32AllGain, XM_U8 u8Mode) {
  ...
  if ((((su8Idx & 0x0F) != u8Idx) || ((su8Idx >> 4) & 0x0F) != u8Idx2) ||
    ...
    {
      sensor_write_register(0x3633,
                            (XM_U32)gau8Logic_1_sc2145H[u8Idx][u8Idx2][0]);
      sensor_write_register(0x3301,
                            (XM_U32)gau8Logic_1_sc2145H[u8Idx][u8Idx2][1]);
      sensor_write_register(0x3622,
                            (XM_U32)gau8Logic_1_sc2145H[u8Idx][u8Idx2][2]);
    } else // B
    {
      sensor_write_register(0x3633,
                            (XM_U32)gau8Logic_0_sc2145H[u8Idx][u8Idx2][0]);
      sensor_write_register(0x3301,
                            (XM_U32)gau8Logic_0_sc2145H[u8Idx][u8Idx2][1]);
      sensor_write_register(0x3622,
                            (XM_U32)gau8Logic_0_sc2145H[u8Idx][u8Idx2][2]);
      sensor_write_register(0x3635,
                            (XM_U32)gau8Logic_0_sc2145H[u8Idx][u8Idx2][3]);
      sensor_write_register(0x366f,
                            (XM_U32)gau8Logic_0_sc2145H[u8Idx][u8Idx2][4]);
    }
...
static const XM_U16 gau16FpsRegAddr[21] = {
    0x3035, 0x3039, 0x3235, 0x3213, 0x3302, 0x3236, 0x3f04,
    0x3f05, 0x3301, 0x3303, 0x3306, 0x330b, 0x3309, 0x330e,
    0x3367, 0x3622, 0x3633, 0x3635, 0x3637, 0x366f, 0x3908,
};
...
STATIC XM_VOID cmos_fps_set(XM_U8 u8Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft) {
  ...
  for (u8i = 0; u8i < 21; u8i++) {
    sensor_write_register((XM_U32)gau16FpsRegAddr[u8i],
                          (XM_U32)gau8FpsRegData[SnsSwitch][u8i][u8Idx]);
  }
...
XM_S32
cmos_init_sensor_exp_function_sc2145H(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc) {
#if 0
...
const ISP_CMOS_AGC_TABLE_S g_stIspAgcTable_sc2145H = {
    ...
    /* sharpen_Kd  */
    {0x30, 0x2C, 0x2C, 0x2C, 0x28, 0x1C, 0x18, 0x10, 0x08, 0x08, 0x08, 0x08,
     0x08, 0x08, 0x08, 0x08},
    ...
    /* snr_thresh 3DNr	 Sf	*/
    {0x00, 0x00, 0x00, 0x04, 0x0C, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10,
     0x10, 0x10, 0x10, 0x10},

    /* DyDpc_thresh 		*/
    {0xB4, 0xB8, 0xD4, 0xD8, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0, 0xE0,
     0xE0, 0xE0, 0xE0, 0xE0},

    /* saturation_strength */
    {0x9A, 0x9A, 0x94, 0x90, 0x88, 0x70, 0x50, 0x30, 0x30, 0x30, 0x30, 0x30,
     0x30, 0x30, 0x30, 0x30},

    /* Blc	*/
    {0x110, 0x110, 0x110, 0x110, 0x110, 0x104, 0x100, 0xE0, 0xE0, 0xE0, 0xE0,
...
const ISP_COLORMATRIX_AUTO_S g_stAwbCcm_sc2145H = {
    5000, {0x000, 383, -157, 30, 0x000, -50, 300, 6, 0x000, 11, -276, 521},
    4000, {0x000, 305, -124, 75, 0x000, -39, 213, 82, 0x000, 19, -311, 548},
    2800, {0x000, 305, -113, 64, 0x000, -49, 200, 105, 0x000, 0, -483, 739}};
...
static const XM_U16 gau16SnsInit_sc2145H[][2] = {
    {0x0100, 0x00}, {0x3039, 0x80}, {0x3034, 0x06}, {0x3035, 0xe2},
    {0x3038, 0x84}, {0x303a, 0xba}, {0x3208, 0x05}, {0x3209, 0x08},
    {0x320a, 0x02}, {0x320b, 0xd8}, {0x320e, 0x02}, {0x320f, 0xee},
    {0x3211, 0x04}, {0x3213, 0x04}, {0x3235, 0x05}, {0x3236, 0xda},
    {0x3302, 0x7d}, {0x3308, 0x1c}, {0x331b, 0x83}, {0x331e, 0x11},
    {0x331f, 0x91}, {0x3320, 0x07}, {0x3321, 0x9c}, {0x3326, 0x00},
    {0x3333, 0x30}, {0x3334, 0x40}, {0x3357, 0x51}, {0x335e, 0x01},
    {0x335f, 0x02}, {0x3366, 0x7c}, {0x3368, 0x04}, {0x3369, 0x00},
    {0x336a, 0x00}, {0x336b, 0x00}, {0x337c, 0x03}, {0x337d, 0x05},
    {0x337f, 0x03}, {0x33a0, 0x04}, {0x3620, 0x28}, {0x3621, 0x20},
    {0x3622, 0xd6}, {0x3625, 0x02}, {0x3630, 0x20}, {0x3631, 0x86},
    {0x3632, 0x28}, {0x3636, 0x25}, {0x3638, 0x1f}, {0x3639, 0x09},
    {0x363b, 0x0c}, {0x366e, 0x08}, {0x3670, 0x02}, {0x3674, 0x20},
    {0x3675, 0x2f}, {0x3676, 0x2f}, {0x367c, 0x07}, {0x367d, 0x0f},
    {0x3802, 0x01}, {0x3e00, 0x00}, {0x3e01, 0x5d}, {0x3e02, 0x80},
    {0x3e03, 0x0b}, {0x3e07, 0x00}, {0x3e08, 0x03}, {0x3e09, 0x10},
    {0x3f00, 0x07}, {0x3f04, 0x02}, {0x3f05, 0xfc}, {0x4500, 0x51},
    {0x3039, 0x51}, {0x3d08, 0x03}, // Zgb
    {0x3640, 0x02},                 // Lycai
    {0x3641, 0x02},                 // Lycai
    ...
    {0x3301, 0x07}, {0x3303, 0x18}, {0x3306, 0x24}, {0x3309, 0x98},
    {0x330b, 0x66}, {0x330e, 0x38}, {0x3367, 0x04}, {0x3622, 0xd6},
    {0x3633, 0x43}, {0x3635, 0x8f}, {0x3637, 0x18}, {0x366f, 0x2a},
    {0x3908, 0x41}, {0x0100, 0x01},
};
