...
STATIC XM_VOID cmos_fps_set(XM_U8 u8Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft) {
  ...
  for (u8i = 0; u8i < u8Num; u8i++) {
    sensor_write_register((XM_U32) * (pu8RegVal + 3 * u8i + 0),
                          (XM_U32) * (pu8RegVal + 3 * u8i + u8Idx));
  }
...
XM_S32
cmos_init_sensor_exp_function_sp140A(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc) {
...
const ISP_CMOS_AGC_TABLE_S g_stIspAgcTable_sp140A = {
    ...
    /* saturation_strength */
    {0x88, 0x88, 0x86, 0x84, 0x80, 0x60, 0x38, 0x20, 0x10, 0x10, 0x10, 0x10,
     0x10, 0x10, 0x10, 0x10},

    /* Blc	*/
    {0x101, 0x101, 0x101, 0x101, 0x101, 0x101, 0x101, 0x101, 0x101, 0x101,
     0x101, 0x101, 0x101, 0x101, 0x101, 0x101},
...
const AE_SENSOR_DEFAULT_V2_S gstAeSnsDef_sp140A = {1,
                                                   HD720P_LINES -
                                                       4, // minInt   	maxInt
                                                   16,
                                                   255, // minAgain maxAgain
                                                   64,
                                                   255,   // minDgain 	maxDgain
                                                   0xFFF, // maxIspGain
                                                   0x01010002, // Update
                                                   {700, 700, 700, 700},
                                                   0x00};
...
const ISP_COLORMATRIX_AUTO_S g_stAwbCcm_sp140A = {
    5000, {0x0000, 370, -140, 26, 0x0000, -65, 367, -46, 0x0000, 16, -197, 437},
    4000, {0x0000, 345, -127, 38, 0x0000, -94, 340, 10, 0x0000, -4, -271, 531},
    2800, {0x0000, 310, -73, 19, 0x0000, -99, 322, 33, 0x0000, -29, -385, 670}};
...
static const XM_U8 gau8SnsInit_sp140A[][2] = {
    {0xfd, 0x00}, {0x2f, 0x20}, {0x2e, 0x01}, {0xfd, 0x01}, {0x06, 0x05}, // gwc
    {0x09, 0x02}, {0x0a, 0x08}, {0x03, 0x00},                             // 0xd
    {0x04, 0xe0},                             // 0x82
    {0x24, 0xff}, {0x35, 0x40}, {0x36, 0x80}, // 0xff
    {0x37, 0x80},                             // 0xff
    {0x38, 0x80},                             // 0xff
    {0x39, 0x80},                             // 0xff
    {0x01, 0x01}, {0xfb, 0x73}, {0xf9, 0x1c}, {0xfd, 0x01}, {0x11, 0x80},
    {0x13, 0x40}, {0x33, 0x30}, {0x16, 0x18}, {0x1c, 0xed}, {0x21, 0x03},
    {0x22, 0x47}, {0x28, 0x04}, {0x29, 0x0e}, {0x2a, 0x1d}, {0x2c, 0xe0},
    {0x30, 0x01}, {0xfd, 0x01}, {0x55, 0x10}, {0x66, 0x56}, {0x68, 0x58},
    {0x58, 0x20}, {0x5b, 0x02}, {0x69, 0x0a}, {0x71, 0x60}, {0x72, 0x0a},
    {0x74, 0x60}, {0x75, 0x0a}, // 0x20
    {0x78, 0x3c}, {0x84, 0x70}, {0x8a, 0x33}, {0x8b, 0x33}, {0xb1, 0x00},
    {0xac, 0x00}, {0xfd, 0x02}, {0x37, 0x40}, // 0xf0
    {0x3a, 0xff}, {0xfd, 0x00}, {0x1b, 0x00}, {0xfd, 0x01}, {0xe7, 0x03},
    {0xe7, 0x00},
#if 0
	// Add @20171228
	{0x24,0x20},
