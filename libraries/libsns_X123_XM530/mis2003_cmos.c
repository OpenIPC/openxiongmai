...
static XM_VOID cmos_dgain_calc_table(XM_U32 u32InTimes,
  ...
  pstAeSnsGainInfo->u32GainDb =
      ((i << 5) | (u32InTimes - 32)) | (pstAeSnsGainInfo->u32SnsTimes << 16);
  return;
}
...
STATIC XM_VOID cmos_gains_update(XM_U32 u32Again, XM_U32 u32Dgain) {
  ...
  // XM_U32 u32AllGain;
  // u32AllGain = (XM_U32)gau16GainTbl[u32Again]*(u32Dgain>>16) >> 5;//64*32 ->
  // 64
...
#else
    5000, // 0.15
    {0x000, 348, -127, 35, 0x000, -91, 372, -25, 0x000, 22, -148, 382},
    4000, // 0.15
    {0x000, 342, -143, 57, 0x000, -95, 369, -18, 0x000, 8, -200, 448},
    2800, // 0.12
    {0x000, 245, -38, 49, 0x000, -144, 397, 3, 0x000, -110, -338, 704}
...
static const XM_U16 gau16SnsInit_MIS2003[][2] = {
    ...
    {0x4034, 0x34},
    {0x4036,
     0xE0}, // sharding消除、第一个斜波拉长500ns、保持STA_DA与EN_RAMP间隔200ns
...
XM_U32 sensor_getlist_MIS2003(XM_U16 *pu16Num) {
  DEBUG(
      "------------- MIS2003 2M 25fps  init ok! (@20191010)----------------\n");
