...
static const ISP_CMOS_AGC_TABLE_S g_stIspAgcTable = {
    ...
    /* sharpen_D	H	*/
    {0x3C, 0x3C, 0x39, 0x37, 0x32, 0x2C, 0x1E, 0x10, 0x06, 0x06, 0x06, 0x06,
     0x06, 0x04, 0x04, 0x04},
    /* sharpen_Ud M	*/
    {0x32, 0x32, 0x30, 0x2E, 0x2A, 0x26, 0x1C, 0x10, 0x02, 0x02, 0x02, 0x02,
     0x02, 0x04, 0x04, 0x04},
    /* sharpen_Kd  */
    {0x44, 0x44, 0x40, 0x3C, 0x32, 0x26, 0x1A, 0x0A, 0x02, 0x08, 0x08, 0x08,
     0x08, 0x08, 0x08, 0x08},
    /* snr_thresh 2DNr		*/
    {0x01, 0x04, 0x08, 0x0E, 0x14, 0x30, 0x48, 0x60, 0x60, 0x60, 0x60, 0x60,
     0x60, 0x60, 0x60, 0x60},
    /* snr_thresh 3DNr	 Tf	*/
    {0x04, 0x08, 0x0A, 0x0C, 0x0E, 0x10, 0x11, 0x12, 0x12, 0x12, 0x12, 0x12,
     0x12, 0x12, 0x12, 0x12},
    /* snr_thresh 3DNr	 Sf	*/
    ...
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00},
    /* saturation_strength */
    {0xB8, 0xB8, 0xB4, 0xB0, 0xA8, 0x96, 0x80, 0x64, 0x10, 0x10, 0x10, 0x10,
     0x10, 0x10, 0x10, 0x10},
    /* Blc	*/
    {0x48, 0x48, 0x48, 0x48, 0x48, 0x48, 0x48, 0x48, 0x10, 0x10, 0x10, 0x10,
     0x10, 0x10, 0x10, 0x10},
    /*Y_Tran gamma*/
...
#if (defined CSTM_AJ)
    5000, // 0.3
    {0x0000, 256, 0, 0, 0x0000, 0, 256, 0, 0x0000, 0, 0, 256},
    4000, // 0.15
    {0x0000, 249, -106, -113, 0x0000, 12, 118, 126, 0x0000, 26, -206, 436},
    2800, // 0.15
    {0x0000, 252, -138, 142, 0x0000, 3, 90, 163, 0x0000, -5, -345, 606}
#else
    5000, // 0.3
    {0x0000, 318, -133, 71, 0x0000, 33, 177, 46, 0x0000, 59, -169, 366},
    4000, // 0.15
    {0x0000, 249, -106, -113, 0x0000, 12, 118, 126, 0x0000, 26, -206, 436},
    2800, // 0.15
    {0x0000, 252, -138, 142, 0x0000, 3, 90, 163, 0x0000, -5, -345, 606}
...
static XM_S32 cmos_get_awb_default(AWB_SENSOR_DEFAULT_S *pstAwbSnsDft) {
  ...
  //	XM_MPI_ISP_Memset((XM_U8*)pstAwbSnsDft, 0,
 ...
 ****************************************************************************/
XM_S32
cmos_init_sensor_exp_function_sc2145(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc) {
...
static const XM_U16 gau16SnsInit_sc2145[][2] = {
    ...
    {0x3001, 0xfe}, {0x3018, 0x33}, {0x3031, 0x0a}, {0x3034, 0x31},
    {0x3035, 0xaa}, {0x3039, 0x00}, {0x303a, 0x36}, {0x303b, 0x06},
    {0x303c, 0x08}, {0x303f, 0x01}, {0x3200, 0x00}, {0x3201, 0x00},
    {0x3204, 0x07}, {0x3205, 0x9f}, {0x3208, 0x07}, {0x3209, 0x88},
    {0x320a, 0x04}, {0x320b, 0x40}, {0x320c, 0x0a}, {0x320d, 0x50},
    {0x3211, 0x10}, {0x3213, 0x10}, {0x3223, 0x50}, {0x322c, 0x07},
    {0x322d, 0xa8}, {0x322e, 0x08}, {0x322f, 0x37}, {0x3300, 0x20},
    {0x3302, 0x28}, {0x3306, 0xd8}, {0x3308, 0x20}, {0x3309, 0x20},
    {0x330a, 0x01}, {0x330b, 0xd0}, {0x330e, 0x20}, {0x331f, 0x17},
    {0x3321, 0x1a}, {0x3333, 0x80}, {0x3334, 0xa0}, {0x3364, 0x05},
    {0x3367, 0x08}, {0x3368, 0x04}, {0x3369, 0x00}, {0x336a, 0x00},
    {0x336b, 0x00}, {0x337f, 0x03}, {0x3400, 0x53}, {0x3401, 0x1e},
    {0x3402, 0x00}, {0x3403, 0x78}, {0x3416, 0x11}, {0x3620, 0xc4},
    {0x3621, 0x18}, {0x3622, 0x0e}, {0x3623, 0x02}, {0x3630, 0x00},
    {0x3631, 0x80}, {0x3632, 0x70}, {0x3633, 0x74}, {0x3635, 0x03},
    {0x3637, 0x83}, {0x3638, 0x84}, {0x3670, 0x0b}, {0x3671, 0xc2},
    {0x3672, 0xc2}, {0x3673, 0x63}, {0x3674, 0xd0}, {0x3675, 0x90},
    {0x3676, 0x0f}, {0x3677, 0x0b}, {0x3678, 0x08}, {0x3679, 0x06},
    {0x367a, 0x07}, {0x367b, 0x1f}, {0x367c, 0x07}, {0x367d, 0x0f},
    {0x367e, 0x07}, {0x367f, 0x1f}, {0x3907, 0x01}, {0x3908, 0xc0},
    {0x3c00, 0x41}, {0x3d08, 0x00}, {0x3e00, 0x00}, {0x3e01, 0x8c},
    {0x3e02, 0x60}, {0x3e03, 0x03}, {0x3e07, 0x00}, {0x3e08, 0x03}, // 0x0b
    {0x3e09, 0x10}, {0x5781, 0x08}, {0x5782, 0x08}, {0x5785, 0x20},
    {0x57a2, 0x01}, {0x57a3, 0xf1}, {0x0100, 0x01},
};
