...
const static XM_U16 gau16LogicAddr[6] = {0x3301, 0x3306, 0x3631,
                                         0x3632, 0x3633, 0x3622};
...
const static XM_U16 gau16LogicAddr_Mipi[3] = {0x3301, 0x3306, 0x3632};
const static XM_U8 gau8Logic_sc2235P_Mipi[6][3] = {
    //  0x3301 0x3306 0x3632
    {0x06, 0x48, 0x08}, {0x14, 0x48, 0x08}, {0x18, 0x48, 0x08},
    {0x13, 0x48, 0x08}, {0xc5, 0x78, 0x48}, {0xc5, 0x78, 0x78}};
    ...
    if (gstSnsAttr.u8InputMode == SENSCONT_MIPI) {
      for (u8i = 0; u8i < 3; u8i++) {
        sensor_write_register((XM_U32)gau16LogicAddr_Mipi[u8i],
                              (XM_U32)gau8Logic_sc2235P_Mipi[u8Idx2][u8i]);
      }
    } else {
      for (u8i = 0; u8i < 6; u8i++) {
        sensor_write_register((XM_U32)gau16LogicAddr[u8i],
                              (XM_U32)gau8Logic_sc2235P[u8Idx][u8Idx2][u8i]);
      }
    }
...
static XM_VOID cmos_fps_set_sc2235P(XM_U8 u8Fps,
                                    AE_SENSOR_DEFAULT_S *pstAeSnsDft) {
...
const AE_SENSOR_DEFAULT_V2_S gstAeSnsDef_sc2235p = {2,
                                                    HD1080P_LINES -
                                                        4, // minInt   	maxInt
                                                    1024,
...
const ISP_CMOS_AGC_TABLE_S g_stIspAgcTable_sc2235P = {
    ...
    /* snr_thresh 2DNr		*/
    {0x00, 0x01, 0x04, 0x10, 0x20, 0x28, 0x60, 0x60, 0x60, 0x40, 0x40, 0x40,
     0x40, 0x40, 0x40, 0x40},

    /* snr_thresh 3DNr	 Tf	*/
    {0x02, 0x03, 0x05, 0x08, 0x0e, 0x14, 0x15, 0x19, 0x19, 0x19, 0x18, 0x18,
     0x18, 0x18, 0x18, 0x18},
    ...
    /* saturation_strength */
    {0xA4, 0xA4, 0x9A, 0x90, 0x88, 0x78, 0x6A, 0x20, 0x20, 0x20, 0x20, 0x20,
     0x20, 0x20, 0x20, 0x20},

    /* Blc	*/
    {0x40, 0x40, 0x40, 0x40, 0x40, 0x38, 0x34, 0x30, 0x30, 0x30, 0x30, 0x30,
     0x30, 0x30, 0x30, 0x30},

    /*Y_Tran gamma*/
    {0x32, 0x32, 0x32, 0x31, 0x2F, 0x2D, 0x38, 0x38, 0x38, 0x38, 0x28, 0x28,
     0x28, 0x28, 0x28, 0x28},
};
...
const ISP_COLORMATRIX_AUTO_S g_stAwbCcm_sc2235P = {
    5000,
    {0x000, 332, -58, -18, 0x000, -50, 391, -85,
     // 0x000,-20, -100, 376
     0x000, 20, -100, 336},
    4000,
    {0x000, 327, -72, 1, 0x000, -46, 362, -60, 0x000, -2, -149, 407},
    2800,
    {0x000, 265, 24, -33, 0x000, -102, 374, -16, 0x000, -54, -272, 582}};

// ExtClk: 37.125M
static const XM_U16 gau16SnsInit_sc2235P_DVP[][2] = {
    {0x0100, 0x00}, {0x3039, 0xa4}, {0x3018, 0x1f}, {0x3019, 0xff},
    {0x301c, 0xb4}, {0x3624, 0x08}, {0x5000, 0x06}, {0x3e08, 0x03},
    {0x3e09, 0x10}, {0x337f, 0x03}, {0x3368, 0x04}, {0x3369, 0x00},
    {0x336a, 0x00}, {0x336b, 0x00}, {0x3367, 0x08}, {0x330e, 0x30},
    {0x3366, 0x7c}, {0x3302, 0x1f}, {0x303f, 0x81}, {0x3907, 0x00},
    {0x3902, 0xC5}, // 0x45
    {0x3905, 0xb8}, {0x3e01, 0x8c}, {0x3e02, 0x10}, {0x3e06, 0x00},
    {0x3908, 0x11}, {0x335e, 0x01}, {0x335f, 0x03}, {0x337c, 0x04},
    {0x337d, 0x06}, {0x33a0, 0x05}, {0x366e, 0x08}, {0x3637, 0x59},
    {0x3326, 0x00}, {0x3303, 0x20}, {0x3638, 0x1f}, {0x3636, 0x25},
    {0x3625, 0x02}, {0x331b, 0x83}, {0x3333, 0x30}, {0x3635, 0xa0},
    {0x363c, 0x05}, {0x3314, 0x13}, {0x363b, 0x0b}, {0x3038, 0xff},
    {0x3639, 0x09}, {0x3621, 0x28}, {0x366f, 0x2f}, {0x3320, 0x01},
    {0x331e, 0x19}, {0x330b, 0xd3}, {0x3620, 0x28}, {0x3309, 0x60},
    {0x331f, 0x59}, {0x3308, 0x10}, {0x320c, 0x08}, {0x320d, 0x98},
    {0x320e, 0x04}, {0x320f, 0x65}, {0x3208, 0x07}, {0x3209, 0x88},
    {0x320a, 0x04}, {0x320b, 0x40}, {0x3211, 0x04}, {0x3213, 0x04},
    {0x3f00, 0x07}, {0x3f04, 0x04}, {0x3f05, 0x28}, {0x3802, 0x01},
    {0x3235, 0x08}, {0x3236, 0xc8}, {0x303b, 0x06}, {0x3034, 0x05},
    {0x3035, 0x82}, {0x33aa, 0x10}, {0x3630, 0x1c}, {0x3039, 0x24},
    {0x303a, 0x36}, {0x3301, 0x10}, {0x3306, 0x48}, {0x3631, 0x84},
    {0x3632, 0x08}, {0x3633, 0x34}, {0x3622, 0xc6}, {0x5780, 0xff},

    {0x3d08, 0x03}, {0x3640, 0x01}, {0x3641, 0x01}, {0x391c, 0x98}, // Blc…¡Œ Ã‚
    {0x0100, 0x01},
};

// ExtClk: 27M MIPI_2Lane
static const XM_U16 gau16SnsInit_sc2235P_Mipi[][2] = {
    {0x0100, 0x00}, {0x3001, 0xfe}, {0x3018, 0x33}, {0x3031, 0x0a},
    {0x3037, 0x20}, {0x3038, 0xff}, {0x3039, 0x22}, {0x303b, 0x06},
    {0x303c, 0x0e}, {0x3200, 0x00}, {0x3201, 0x00}, {0x3202, 0x00},
    {0x3203, 0x00}, {0x3204, 0x07}, {0x3205, 0x8f}, {0x3206, 0x04},
    {0x3207, 0x47}, {0x3208, 0x07}, {0x3209, 0x88}, {0x320a, 0x04},
    {0x320b, 0x40}, {0x320c, 0x0a}, {0x320d, 0x50}, {0x320e, 0x04},
    {0x320f, 0x65}, {0x3211, 0x04}, {0x3213, 0x04}, {0x3222, 0x29},
    {0x3235, 0x08}, {0x3236, 0xc8}, {0x3301, 0x06}, {0x3302, 0x1f},
    {0x3303, 0x20}, {0x3306, 0x48}, {0x3308, 0x10}, {0x3309, 0x60},
    {0x330b, 0xd3}, {0x330e, 0x30}, {0x3314, 0x08}, {0x331b, 0x83},
    {0x331e, 0x19}, {0x331f, 0x59}, {0x3320, 0x01}, {0x3326, 0x00},
    {0x3333, 0x30}, {0x335e, 0x01}, {0x335f, 0x03}, {0x3366, 0x7c},
    {0x3367, 0x08}, {0x3368, 0x04}, {0x3369, 0x00}, {0x336a, 0x00},
    {0x336b, 0x00}, {0x337c, 0x04}, {0x337d, 0x06}, {0x337f, 0x03},
    {0x33a0, 0x05}, {0x33aa, 0x10}, {0x360f, 0x01}, {0x3620, 0x28},
    {0x3621, 0x28}, {0x3622, 0x06}, {0x3624, 0x08}, {0x3625, 0x02},
    {0x3630, 0x1c}, {0x3631, 0x84}, {0x3632, 0x08}, {0x3633, 0x4f},
    {0x3635, 0xa0}, {0x3636, 0x25}, {0x3637, 0x59}, {0x3638, 0x1f},
    {0x3639, 0x09}, {0x363a, 0x9f}, {0x363b, 0x0b}, {0x363c, 0x05},
    {0x366e, 0x08}, {0x366f, 0x2f}, {0x3670, 0x0c}, {0x3671, 0xc6},
    {0x3672, 0x06}, {0x3673, 0x16}, {0x3677, 0x84}, {0x3678, 0x88},
    {0x3679, 0x88}, {0x367a, 0x28}, {0x367b, 0x3f}, {0x367e, 0x08},
    {0x367f, 0x28}, {0x3690, 0x34}, {0x3691, 0x11}, {0x3692, 0x42},
    {0x369c, 0x08}, {0x369d, 0x28}, {0x3802, 0x01}, {0x3901, 0x02},
    {0x3902, 0xC5}, // 0x45
    {0x3905, 0x98}, {0x3907, 0x00}, {0x3908, 0x11}, {0x391b, 0x80},
    ...
    {0x4603, 0x00}, {0x4827, 0x48}, {0x4837, 0x35}, {0x5000, 0x06},
    {0x5780, 0xff}, {0x5781, 0x04}, {0x5785, 0x18},
...
XM_U32 sensor_getlist_sc2235P(XM_U16 *pu16Num) {
  if (gstSnsAttr.u8InputMode == SENSCONT_MIPI) {
    ...
    *pu16Num = sizeof(gau16SnsInit_sc2235P_Mipi) /
