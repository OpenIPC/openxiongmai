...
int sensor_register_callback(ISP_DEV IspDev) {
  ...
  case SENSOR_CHIP_SC5332:
    cmos_init_sensor_exp_function_sc5332(&(stIspRegister.stSnsExp));
    cmos_init_ae_exp_function_sc5332(&(stAeRegister.stSnsExp));
    cmos_init_awb_exp_function_sc5332(&(stAwbRegister.stSnsExp));
    break;
...
int sensor_get_chip(I2C_DATA_S *pstI2CData) {
  ...
  if (s32Id == 0x5235) {
    ...
    goto RET_TAB;
  } else if (s32Id == 0xce1a) {
    gSensorChip = SENSOR_CHIP_SC5332;
    goto RET_TAB;
...
void sensor_set_chip(XM_S32 s32SnsChip) {
  ...
  case SENSOR_CHIP_SC5332:
    ...
    break;
  default:
    fprintf(stderr, "1sensor_set_chip not support this sensor %d!\n",
            s32SnsChip);
    break;
  }
...
int sensor_set_ob(XM_U16 u16Ob) { return XM_SUCCESS; }
...
*************************************************************************************/
void sensor_restart(XM_U8 u8Mode) { return; }
