...
typedef enum xmVI_DATA_YUV_SEQ_E {
  /*The input sequence of the second component(only contains u and v) in BT.1120
     mode */
...
typedef enum xmVI_VSYNC_E {
  ...
  VI_VSYNC_PULSE, /* Pusle/effective mode:a pusle or an effective signal means a
                     new frame or a field */
} VI_VSYNC_E;

/* Polarity of the vertical synchronization signal */
typedef enum xmVI_VSYNC_NEG_E {
  VI_VSYNC_NEG_HIGH = 0, /*if VIU_VSYNC_E = VIU_VSYNC_FIELD,then the vertical
                            synchronization signal of even field is high-level,
                               if VIU_VSYNC_E = VIU_VSYNC_PULSE,then the
                            vertical synchronization pulse is positive pulse.*/
...
typedef enum xmVI_HSYNC_NEG_E {
  VI_HSYNC_NEG_HIGH = 0, /*if VI_HSYNC_E = VI_HSYNC_VALID_SINGNAL,then the valid
                            horizontal synchronization signal is high-level; if
...
typedef enum xmVI_VSYNC_VALID_NEG_E {
  VI_VSYNC_VALID_NEG_HIGH =
      0, /*if VI_VSYNC_VALID_E = VI_VSYNC_NORM_PULSE,a positive pulse means
            ...
            high-level */
  VI_VSYNC_VALID_NEG_LOW /*if VI_VSYNC_VALID_E = VI_VSYNC_NORM_PULSE,a negative
                            pulse means vertical synchronization pulse; if
                            VI_VSYNC_VALID_E = VI_VSYNC_VALID_SINGAL,the valid
                            vertical synchronization signal is low-level */
...
typedef struct xmVI_TIMING_BLANK_S {
  ...
  XM_U32 u32VsyncVact; /* Vertical effetive width of one frame or odd-field
                          frame picture */
  XM_U32 u32VsyncVbb; /* Vertical back blanking height of one frame or odd-field
                         frame picture */
  XM_U32
  u32VsyncVbfb; /* Even-field vertical front blanking height when input mode
                   is interlace (invalid when progressive input mode) */
  XM_U32 u32VsyncVbact; /* Even-field vertical effetive width when input mode is
...
typedef enum xmBT656_FIELD_POLAR_E {
  ...
  BT656_FIELD_POLAR_NSTD /* the non-standard BT.656 mode,the first filed F=1,the
                            second filed F=0*/
} BT656_FIELD_POLAR_E;
...
typedef struct xmVI_CHN_ATTR_S {
  ...
  SIZE_S stDestSize; /* Target picture size.
                        ...
                        stCapRect's u32Width and u32Height, because primary
                        channel doesn't have scale capability. Additionally, it
                        is a static attribute, That is, the value of stDestSize
                        can be changed only after primary and secondary channels
                        are disabled. For secondary channels, stDestSize is a
                        dynamic attribute */
  ...
  XM_S32 s32SrcFrameRate;  /* Source frame rate. The value -1 indicates that the
...
typedef struct xmVI_USERPIC_ATTR_S {
  ...
  union {
    ...
    VI_USERPIC_BGC_S stUsrPicBg; /* Information about a background picture only
                                    with a color */
  } unUsrPic;
} VI_USERPIC_ATTR_S;
